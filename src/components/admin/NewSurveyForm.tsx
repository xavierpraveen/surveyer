...
export default function NewSurveyForm({ roleOptions }: NewSurveyFormProps) {
  ...
  function toggleRole(roleId: string) {
    setForm((prev) => {
      const exists = prev.target_role_ids.includes(roleId)
      return {
        ...prev,
        target_role_ids: exists
          ? prev.target_role_ids.filter((id) => id !== roleId)
          : [...prev.target_role_ids, roleId],
      }
    })
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-surface border border-border rounded-lg shadow-sm p-6 space-y-5 max-w-xl">
      ...
      <div>
        <label className="block text-sm font-medium text-fg mb-2">Audience roles</label>
        <p className="text-xs text-fg-subtle mb-2">
          Leave unselected to target all roles.
        </p>
        {roleOptions.length === 0 ? (
          <p className="text-sm text-fg-subtle">No roles configured. Survey will be available to all roles.</p>
        ) : (
          <div className="max-h-44 overflow-y-auto rounded-md border border-border p-2 space-y-1">
            {roleOptions.map((role) => (
              <label key={role.id} className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={form.target_role_ids.includes(role.id)}
